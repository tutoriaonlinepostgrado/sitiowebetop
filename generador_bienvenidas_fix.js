(function(){
  window.enviarSharePoint = async function(){
    ...
    try{
      const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(window.datosFinales)});
    ...
    }catch(err){if(spResult) spResult.innerHTML='<div class="alert error">Error de red: '+err.message+'</div>';}
  };
  ...
  window.buscarCurso = function(campo){
    ...
    const fuente=Array.isArray(window.nrcsObData)&&window.nrcsObData.length?window.nrcsObData:(Array.isArray(window.nrcsData)?window.nrcsData:[]);
